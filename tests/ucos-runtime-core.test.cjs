...
(async()=>{
 ...
 const processes=Core.createProcessManager(resources);const p=processes.spawn('system.files');assert.equal(processes.get(p.pid).state,'running');assert.equal(processes.suspend(p.pid,'test').state,'suspended');assert.equal(processes.resume(p.pid).state,'running');assert.equal(processes.crash(p.pid,'boom').state,'crashed');processes.heartbeat(p.pid,{tick:1});assert.equal(processes.get(p.pid).metadata.tick,1);await processes.terminate(p.pid);assert.equal(processes.size(),0);assert.equal(processes.history().length,1);
 console.log('UCOS process lifecycle suspend resume crash history passed');

 let calls=0,issued=0,finalized=0,audited=0;const broker=Core.createCapabilityBroker({manifests,grants,execute:async c=>{calls++;return c},prompt:async()=>({state:'granted',scope:'once'}),authorize:async()=>{issued++;return{id:'lease-1'}},finalize:async()=>{finalized++},audit:()=>audited++});
 const r=await broker.request({appId:'third.demo',capabilityId:'media.camera',operation:'execute',args:{x:1}});assert.equal(r.capabilityId,'media.camera');assert.equal(r.meta.capabilityLeaseId,'lease-1');assert.equal(calls,1);assert.equal(issued,1);assert.equal(finalized,1);assert(audited>=1);assert.equal((await grants.check('third.demo','media.camera')).state,'prompt');
