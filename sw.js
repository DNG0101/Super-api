...
self.addEventListener('message',event=>{const port=event.ports?.[0];if(event.data?.type==='super-api-ping'){port?.postMessage({type:'super-api-pong',received:event.data.time,responded:Date.now()});return;}if(event.data?.type==='super-api-realm-scan'){port?.postMessage({realm:'ServiceWorkerGlobalScope',constructor:self.constructor?.name||null,globals:Object.getOwnPropertyNames(self).sort(),members:realmMembers(self),registration:realmMembers(self.registration),clients:realmMembers(self.clients),navigator:realmMembers(self.navigator),caches:realmMembers(self.caches),crypto:realmMembers(self.crypto),performance:realmMembers(self.performance)});}});
