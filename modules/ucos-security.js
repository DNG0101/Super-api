...
async function restoreAudit(){try{const rows=await Storage?.get?.(AUDIT_NS,AUDIT_KEY,{preferred:['indexeddb','local','memory']})||[];for(const row of [...rows].reverse())audit.append(row.type,row.data,row.severity)}catch{}}
...
function health(){return{version:5,identityReady:Boolean(identityPromise),tokens:tokens.size(),quotaBuckets:quotas.size(),replayEntries:replay.size(),auditEvents:audit.size(),seenPeers:peerSeen.size}}
