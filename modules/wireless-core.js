(function(root,factory){
  ...
  function networkSnapshot(c){
    if(!c) return {supported:false};
    const keys=['type','effectiveType','downlink','downlinkMax','rtt','saveData'];
    const out={supported:true};
    for(const k of keys){try{if(k in c)out[k]=c[k]}catch{}}
    return out;
  }
  function parseIceCandidate(input){
    const raw=String(input||'').replace(/^a=/,'').trim();
    if(!raw.startsWith('candidate:')) return {valid:false,raw};
    const p=raw.split(/\s+/);
    const typ=p.indexOf('typ');
    const tcptype=p.indexOf('tcptype');
    const raddr=p.indexOf('raddr');
    const rport=p.indexOf('rport');
    return {
      valid:p.length>=8,
      foundation:(p[0]||'').slice(10),
      component:p[1]||null,
      protocol:(p[2]||'').toLowerCase()||null,
      priority:Number(p[3])||0,
      address:p[4]||null,
      port:Number(p[5])||0,
      type:typ>=0?p[typ+1]||null:null,
      tcpType:tcptype>=0?p[tcptype+1]||null:null,
      relatedAddress:raddr>=0?p[raddr+1]||null:null,
      relatedPort:rport>=0?Number(p[rport+1])||0:null
    };
  }
  function redactIce(c){
    if(!c||typeof c!=='object')return c;
    const x={...c};
    if(x.address)x.address='[redacted]';
    if(x.relatedAddress)x.relatedAddress='[redacted]';
    delete x.raw;
    return x;
  }
  function capabilityMatrix(env={}){
    ...
    return {
      ...
      nfc:has('nfc'),
      networkInformation:has('networkInformation'),
      webrtc:has('webrtc'),
      presentation:has('presentation'),
      remotePlayback:has('remotePlayback'),
      webSocket:has('webSocket'),
      webTransport:has('webTransport'),
      geolocation:has('geolocation'),
      usbBridge:has('usb'),
      hidBridge:has('hid'),
      serialBridge:has('serial')
    };
  }
  function combinations(families=FAMILIES,ops=['detect','inspect','connect']){
    const out=[];
    for(const f of families)for(const op of ops)out.push({family:f.id,op,direct:f.direct,raw:f.raw});
    return out;
  }
