...
async function readBlob(path){path=normalize(path);return withLock(path,async()=>{const {dir,name}=await resolveParent(path);const h=await dir.getFileHandle(name);return h.getFile()},{mode:'shared'})}
...
async function remove(path,{recursive=false}={}){path=normalize(path);if(path==='/'||ROOT_DIRS.includes(base(path))&&split(path).length===1)throw new Error('protected-path');return withLock(path,async()=>{const {dir,name}=await resolveParent(path);await dir.removeEntry(name,{recursive});return true})}
async function copyUnlocked(src,dst){const s=await stat(src);if(!s)throw new Error('source-not-found');if(s.kind==='file'){const f=await readBlob(src);return writeBlobUnlocked(dst,f,{type:f.type})}await mkdir(dst,{recursive:true});for(const item of await list(src))await copyUnlocked(item.path,normalize(`${dst}/${item.name}`));return stat(dst)}
async function copy(src,dst){src=normalize(src);dst=normalize(dst);return withLocks([src,dst],()=>copyUnlocked(src,dst))}
async function move(src,dst){src=normalize(src);dst=normalize(dst);return withLocks([src,dst],async()=>{const r=await copyUnlocked(src,dst);const {dir,name}=await resolveParent(src);await dir.removeEntry(name,{recursive:true});return r})}
async function rename(src,newName){if(!newName||/[\\/\0]/.test(newName))throw new Error('invalid-name');return move(src,normalize(`${parent(src)}/${newName}`))}
...
function health(){return{version:5,opfs:Boolean(navigator.storage?.getDirectory),locks:Boolean(navigator.locks?.request),roots:[...ROOT_DIRS],mounts:listMounts()}}
const api=Object.freeze({normalize,split,parent,base,init,health,stat,exists,list,mkdir,writeBlob,writeAtomic,writeText,writeJSON,readBlob,readText,readJSON,readRange,remove,copy,move,rename,trash,emptyTrash,quota,requestPersistence,mountDirectory,requestExternalMount,unmount,listMounts,withLock});
