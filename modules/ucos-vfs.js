...
async function readBlobUnlocked(path){const {dir,name}=await resolveParent(path);const h=await dir.getFileHandle(name);return h.getFile()}
...
async function remove(path,{recursive=false}={}){path=normalize(path);if(path==='/'||ROOT_DIRS.includes(base(path))&&split(path).length===1)throw new Error('protected-path');return withLock(path,()=>removeUnlocked(path,{recursive}))}
...
async function move(src,dst){src=normalize(src);dst=normalize(dst);return withLocks([src,dst],async()=>{const r=await copyUnlocked(src,dst);await removeUnlocked(src,{recursive:true});return r})}
