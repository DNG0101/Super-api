...
function normalizeManifest(input={}){
 ...
 return Object.freeze({schemaVersion:3,id:appId,name:text(input.name||appId),version:text(input.version||'0.0.0'),description:text(input.description),runtime,entry:text(input.entry),icon:text(input.icon),singleton:input.singleton!==false,capabilities,fileTypes:uniq(input.fileTypes),protocols:uniq(input.protocols),keywords:uniq(input.keywords),system:Boolean(input.system),lifecycle:Object.freeze(lifecycle),metadata:clone(input.metadata||{})});
}
...
function createGrantStore(adapter=null){
 ...
 const normalizeCtx=ctx=>({operation:text(ctx?.operation||'execute')||'execute',nodeId:text(ctx?.nodeId||ctx?.targetNodeId||'*')||'*',resourceScope:text(ctx?.resourceScope||ctx?.resource||'*')||'*'});
 const key=(app,cap,ctx={})=>{const c=normalizeCtx(ctx);return`${text(app)}::${text(cap)}::${c.operation}::${c.nodeId}::${c.resourceScope}`};
 const legacyKey=(app,cap)=>`${text(app)}::${text(cap)}`;
 ...
 async function migrateLegacy(appId,capabilityId,ctx){const old=await read(legacyKey(appId,capabilityId));if(!old)return null;const op=text(old.operation||'execute')||'execute';if(op!==ctx.operation)return null;const row={...old,appId:text(appId),capabilityId:text(capabilityId),operation:op,nodeId:text(old.nodeId||'*')||'*',resourceScope:text(old.resourceScope||'*')||'*',migratedFromLegacy:true};await write(key(appId,capabilityId,row),row);await del(legacyKey(appId,capabilityId));return row}
 return Object.freeze({
  async get(appId,capabilityId,ctx={}){const c=normalizeCtx(ctx),row=await read(key(appId,capabilityId,c))||await migrateLegacy(appId,capabilityId,c);return clone(row)},
  async set(appId,capabilityId,{state='granted',scope='session',expiresAt=null,reason='',operation='execute',nodeId='*',resourceScope='*'}={}){if(!['granted','denied','prompt'].includes(state))throw new Error('invalid-grant-state');if(!['once','session','persistent'].includes(scope))throw new Error('invalid-grant-scope');const c=normalizeCtx({operation,nodeId,resourceScope}),row={appId:text(appId),capabilityId:text(capabilityId),state,scope,expiresAt:expiresAt==null?null:Number(expiresAt),reason:text(reason),operation:c.operation,nodeId:c.nodeId,resourceScope:c.resourceScope,updatedAt:Date.now()};await write(key(row.appId,row.capabilityId,row),row);return clone(row)},
  async revoke(appId,capabilityId,ctx=null){if(ctx&&Object.keys(ctx).length)return del(key(appId,capabilityId,ctx));let n=0;for(const[k,v]of await entries())if(v?.appId===appId&&v?.capabilityId===capabilityId){await del(k);n++}await del(legacyKey(appId,capabilityId));return n>0},
  async revokeApp(appId){let n=0;for(const[k,v]of await entries())if(v?.appId===appId||String(k).startsWith(`${appId}::`)){await del(k);n++}return n},
  async check(appId,capabilityId,ctx={}){const c=normalizeCtx(ctx);let g=await read(key(appId,capabilityId,c));if(!g)g=await migrateLegacy(appId,capabilityId,c);if(!g)return{state:'prompt',scope:'session',operation:c.operation,nodeId:c.nodeId,resourceScope:c.resourceScope};if(g.expiresAt&&Date.now()>g.expiresAt){await del(key(appId,capabilityId,c));return{state:'prompt',scope:'session',operation:c.operation,nodeId:c.nodeId,resourceScope:c.resourceScope}}return clone(g)},
  async list(appId=''){return(await entries()).map(([,v])=>clone(v)).filter(v=>v&&(!appId||v.appId===appId))},
  async clearSession(){for(const[k,v]of await entries())if(v?.scope==='session'||v?.scope==='once')await del(k);return true},
  key,normalizeContext:normalizeCtx
 })
}
...
function createCapabilityBroker({manifests,grants,execute,prompt,authorize=null,finalize=null,audit=null}={}){
 ...
 async function request({appId,capabilityId,operation='execute',args={},mode='auto',targetNodeId='',meta={},signal=null}={}){
  ...
  const grantCtx={operation,nodeId:targetNodeId||'*',resourceScope:meta?.resourceScope||'*'};
  let grant=await grants.check(appId,capabilityId,grantCtx);
  if(grant.state==='denied'){log('capability-denied',{appId,capabilityId,operation,reason:grant.reason,...grantCtx},'warn');throw new Error(`capability-denied:${capabilityId}`)}
  if(grant.state!=='granted'){
   if(manifest.system)grant=await grants.set(appId,capabilityId,{state:'granted',scope:'session',reason:'system-app-declaration',...grantCtx});
   else if(typeof prompt==='function'){
    const decision=await prompt({appId,manifest,capabilityId,operation,mode,targetNodeId,meta:clone(meta),resourceScope:grantCtx.resourceScope});
    if(signal?.aborted)throw signal.reason||new DOMException('Aborted','AbortError');
    if(!decision||decision.state!=='granted'){await grants.set(appId,capabilityId,{state:'denied',scope:decision?.scope||'session',reason:decision?.reason||'user-denied',...grantCtx});log('capability-denied',{appId,capabilityId,operation,reason:decision?.reason||'user-denied',...grantCtx},'warn');throw new Error(`capability-denied:${capabilityId}`)}
    grant=await grants.set(appId,capabilityId,{...decision,...grantCtx})
   } else throw new Error(`capability-grant-required:${capabilityId}`)
  }
  const auth=typeof authorize==='function'?await authorize({appId,capabilityId,operation,mode,targetNodeId,manifest,grant,meta:clone(meta),resourceScope:grantCtx.resourceScope}):null;
  const command={capabilityId,operation,args,mode,targetNodeId,meta:{...meta,appId,...(auth?.id?{capabilityLeaseId:auth.id}:{})},signal};
  const started=Date.now();let result,error=null;
  try{result=await execute(command,{signal});log('capability-executed',{appId,capabilityId,operation,mode,targetNodeId,resourceScope:grantCtx.resourceScope,durationMs:Date.now()-started});return result}
  catch(e){error=e;log('capability-error',{appId,capabilityId,operation,mode,targetNodeId,resourceScope:grantCtx.resourceScope,error:e?.message||String(e),durationMs:Date.now()-started},e?.name==='AbortError'?'info':'warn');throw e}
  finally{try{await finalize?.({auth,appId,capabilityId,operation,mode,targetNodeId,result,error,meta:clone(meta),resourceScope:grantCtx.resourceScope})}catch{}if(grant.scope==='once')await grants.revoke(appId,capabilityId,grantCtx)}
 }
