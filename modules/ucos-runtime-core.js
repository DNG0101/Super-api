...
function normalizeManifest(input={}){
  ...
  return Object.freeze({
    schemaVersion:2,id:appId,name:text(input.name||appId),version:text(input.version||'0.0.0'),description:text(input.description),
    ...
    system:Boolean(input.system),lifecycle:Object.freeze(lifecycle),metadata:clone(input.metadata||{})
  });
}
function validateManifest(input){try{const manifest=normalizeManifest(input);const errors=[];if(manifest.runtime==='sandbox'&&!manifest.entry)errors.push('sandbox-entry-required');if(!/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(manifest.version))errors.push('invalid-semver');return{valid:errors.length===0,errors,manifest}}catch(e){return{valid:false,errors:[e.message],manifest:null}}}
...
function createProcessManager(resources){
  ...
  function spawn(appId,{runtime='system',metadata={}}={}){const now=Date.now(),pid=id('proc');const row={pid,appId:text(appId),runtime:text(runtime),state:'running',startedAt:now,updatedAt:now,heartbeatAt:now,suspendedAt:null,restartCount:0,metadata:clone(metadata),error:null};map.set(pid,row);return snapshot(row)}
  function setState(pid,state,error=null){const row=map.get(pid);if(!row)return null;row.state=text(state);row.updatedAt=Date.now();row.error=error?text(error):null;if(state==='suspended')row.suspendedAt=Date.now();if(state==='running')row.suspendedAt=null;return snapshot(row)}
  ...
  function suspend(pid,reason='suspended'){const row=map.get(pid);if(!row||row.state==='terminated')return null;return setState(pid,'suspended',reason)}
  function resume(pid){const row=map.get(pid);if(!row||row.state==='terminated')return null;return setState(pid,'running',null)}
  function crash(pid,error='app-crash'){const row=map.get(pid);if(!row)return null;return setState(pid,'crashed',error)}
...
function createCapabilityBroker({manifests,grants,execute,prompt,authorize=null,finalize=null,audit=null}={}){
  ...
  async function request({appId,capabilityId,operation='execute',args={},mode='auto',targetNodeId='',meta={}}={}){
    ...
    const auth=typeof authorize==='function'?await authorize({appId,capabilityId,operation,mode,targetNodeId,manifest,grant}):null;
    ...
    finally{try{await finalize?.({auth,appId,capabilityId,operation,mode,targetNodeId,result,error})}catch{}if(grant.scope==='once')await grants.revoke(appId,capabilityId)}
  }
