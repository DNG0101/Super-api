...
const BOOT='super-api-ucos-v5-boot',CRASHES='super-api-ucos-v5-crashes',SESSION='super-api-ucos-v5-session';
...
const previous=read(BOOT,null),recentCrashes=(read(CRASHES,[])||[]).filter(t=>now-Number(t)<5*60*1000);
...
let safeMode=forced||interrupted||recentCrashes.length>=3;
...
function markReady(){ready=true;write(BOOT,{state:'ready',time:Date.now()});emit('ready',{safeMode});return true}
function recordCrash(reason='runtime-crash'){crashed=true;const list=(read(CRASHES,[])||[]).filter(t=>Date.now()-Number(t)<5*60*1000);list.push(Date.now());write(CRASHES,list.slice(-10));if(list.length>=3){safeMode=true;globalThis.__UCOS_SAFE_MODE=true}write(BOOT,{state:'crashed',time:Date.now(),reason:String(reason)});emit('crash',{reason:String(reason),count:list.length,safeMode});return list.length}
...
function exitSafeMode(){safeMode=false;globalThis.__UCOS_SAFE_MODE=false;clearCrashHistory();write(BOOT,{state:'ready',time:Date.now(),reason:'safe-mode-exit'});emit('safe-mode',{enabled:false});return true}
...
function health(){return{version:5,ready,crashed,safeMode,interruptedBoot:interrupted,recentCrashCount:recentCrashes.length,hasSession:Boolean(loadSession())}}
